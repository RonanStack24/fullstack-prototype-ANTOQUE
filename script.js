...
function seedData() {
  db.accounts.push({
    firstName: "Admin",
    lastName: "User",
    email: "ucadmin@gmail.com",
    password: "waypassword",
    role: "Admin",
    verified: true,
  });
  db.departments.push(
...
function handleRouting() {
  ...
  if (
    (protectedRoutes.includes(path) || adminRoutes.includes(path)) &&
    !currentUser
  ) {
    navigateTo("login");
    return;
...
if (document.getElementById("edit-profile-form")) {
  document
    ...
    .addEventListener("submit", (e) => {
      ...
      // Find user index
      const userIndex = db.accounts.findIndex(
...
function renderAccounts() {
  document.getElementById("accounts-table-body").innerHTML = db.accounts
    .map(
      (acc, index) => `
        <tr>
...
function prepareAddAccount() {
  ...
  document.querySelector("#accountModal .modal-title").innerText =
    "Add Account";
...
function prepareAddEmployee() {
  ...
  document.querySelector("#employeeModal .modal-title").innerText =
    "Add Employee";
...
function renderDepartments() {
  ...
  tbody.innerHTML = db.departments
    .map(
      (dept, index) => `
        <tr>
            <td>${dept.name}</td>
...
function prepareAddDepartment() {
  ...
  document.querySelector("#departmentModal .modal-title").innerText =
    "Add Department";
...
function renderRequests() {
  ...
  document.getElementById("requests-table-body").innerHTML = myRequests
    .map(
      (req) => `
        <tr>
            <td>${req.date}</td>
...
document.getElementById("request-form").addEventListener("submit", (e) => {
  ...
  const items = [];
  document
    .querySelectorAll("#req-items-container .input-group")
    .forEach((row) => {
